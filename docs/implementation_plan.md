...
> **구현 완료**: Welcome Page(프로토콜 선택 버튼 바 + 컨텍스트 기반 입력 폼)와 Tab Bar 디자인 고도화가 현재 `src/ui/view.rs`(렌더링), `src/app/update.rs`(메시지 처리), `src/app/state.rs`(상태)에 반영되어 있습니다. RDP 프로토콜도 선택기에 포함되어 있습니다.
...
현재 구조에서는 `src/ui/view.rs`의 `view` 함수 내 `tab_bar` 렌더링 부분을 뜯어고칩니다.
