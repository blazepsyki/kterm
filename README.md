...
- 현재 결합 지점: 프레임 처리 공통 경로인 `app/update.rs` 안에 VNC 전용 휴리스틱이 남아 있으며, `session.name.starts_with("VNC:")`와 `session.vnc_rect_only_streak`로 분기합니다.
- 현재 결합 지점: `Session`은 공통 `RemoteDisplay` 세션이면서도 `rdp_secure_attention_active`, `vnc_rect_only_streak` 같은 프로토콜별 상태를 함께 보관합니다.
