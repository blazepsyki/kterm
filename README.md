...
하나의 GUI 안에서 SSH, Telnet, 로컬 PowerShell 세션을 탭으로 관리하고, 직접 구현한 터미널 에뮬레이터로 화면을 렌더링합니다.

## 프로젝트 개요

`kterm`은 "연결(SSH/Telnet/Local)"과 "터미널 렌더링"을 분리한 구조를 가진 Rust 데스크톱 앱입니다.
...
- Telnet 연결
  ...
  - 윈도우 크기 협상(NAWS) 전송
- 로컬 셸 연결
  - Windows에서 `portable-pty`로 `powershell.exe` 실행
- 터미널 UX
...
- `src/connection/mod.rs`
  - 공통 이벤트/입력 타입 정의
  - `ConnectionEvent`: Connected/Data/Disconnected/Error
  - `ConnectionInput`: Data/Resize
...
1. 사용자가 Welcome 화면에서 SSH/Telnet/Local 중 하나를 선택
2. 연결 모듈이 비동기 스트림으로 `ConnectionEvent`를 발행
3. `main.rs`가 이벤트를 수신하여 active 세션의 `TerminalEmulator`에 반영
4. 터미널 출력 렌더링 + 필요 시 응답 시퀀스(예: DSR) 재전송
5. 사용자 키입력/붙여넣기는 `ConnectionInput::Data`로 연결 모듈에 전달
...
- Rust (stable)
- Cargo
- Windows 환경 (로컬 PowerShell 기능 기준)
